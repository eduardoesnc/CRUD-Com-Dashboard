...
const filterUsersByMonth = (users: any[], month: number) => {
  ...
  return users.filter((user: { gender: string; createdAt: Date; }) => {
    ...
    return createdAt >= startOfMonth && createdAt < endOfMonth && user.gender == 'masc';
  });
};
const MascUserCountCard: React.FC = () => {
  ...
  return (
    <Card className='h-32 flex justify-center items-center'>
      <CardContent className='text-center'>
        <Typography variant="h5" className='text-xl'>Cadastros com Sexo Masculinos em {months[selectedMonth]}</Typography>
...
export default MascUserCountCard;
